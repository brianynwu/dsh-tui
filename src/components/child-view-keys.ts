...
export class ChildViewKeys implements Component, Focusable {
  ...
  constructor(private readonly keymap: TuiKeymap, private readonly back: () => void) {}

  render(_width: number): string[] { return [] }
  invalidate(): void {}

  handleInput(data: string): void {
    if (this.keymap.resolve(data, 'subagentBrowser') === 'subagentBack'
      || matchesKey(data, Key.escape) || matchesKey(data, Key.ctrl('c'))) this.back()
  }
}
